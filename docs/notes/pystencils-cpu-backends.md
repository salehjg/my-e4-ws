...
|---------------------|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `GenericCPU`        | scalar (+OpenMP)     | [`submodules/pystencils/src/pystencils/backend/platforms/generic_cpu.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/generic_cpu.py) |
| `CurrentCPU`        | auto-detect          | resolved at codegen time                                                                                                                                                    |
| `X86_SSE`           | 128 b                | [`submodules/pystencils/src/pystencils/backend/platforms/x86.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/x86.py) (`SSE`)         |
| `X86_AVX`           | 256 b                | [`submodules/pystencils/src/pystencils/backend/platforms/x86.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/x86.py) (`AVX`)         |
| `X86_AVX512`        | 512 b                | [`submodules/pystencils/src/pystencils/backend/platforms/x86.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/x86.py) (`AVX512`)      |
| `X86_AVX512_FP16`   | 512 b + fp16         | [`submodules/pystencils/src/pystencils/backend/platforms/x86.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/x86.py) (`AVX512_FP16`) |
| `ARM_NEON`          | 64/128 b             | [`submodules/pystencils/src/pystencils/backend/platforms/neon.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/neon.py)              |
| `ARM_NEON_FP16`     | 64/128 b + fp16      | [`submodules/pystencils/src/pystencils/backend/platforms/neon.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/neon.py) (fp16)       |
| `ARM_SVE`           | scalable             | [`submodules/pystencils/src/pystencils/backend/platforms/sve.py`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/sve.py)                |
...
returns width/itemsize for fixed-width ISAs and **raises `CodegenError` for `ARM_SVE`** —
SVE forces the user to specify a lane count explicitly. Auto-detect logic at
...
Each platform implements `SelectIntrinsics*` that maps IR nodes
(`PsAdd`, `PsLoad`, `PsStore`, `PsBroadcast`, math fns, gather/scatter,
horizontal reductions) to the ISA's intrinsics and emits the right header via
...
- `X86VectorArch` enum at [`x86.py:42-113`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/x86.py#L42-113):
  `SSE=128`, `AVX=256`, `AVX512=512`, `AVX512_FP16=513`. Helpers `intrin_prefix()`
  (`_mm`/`_mm256`/`_mm512`), `intrin_suffix()` (`ps`/`pd`/`epi32`/...),
  `intrin_type()` (`__m128`/`__m256i`/...).
...
- Load: `_mm{,256,512}_loadu_*` packed; `_mm*_i32gather_*` strided.
  Store dual. Broadcast via `_mm*_set1_*`.
- No predication exposed (mask registers used internally on AVX-512 only).
...
- `NeonCpu(enable_fp16: bool)` at [`neon.py:43-72`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/neon.py#L43-72);
  `SelectIntrinsicsNeon` at [`neon.py:102-426`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/backend/platforms/neon.py#L102-426).
- Types `int32x4_t` etc.; Q-suffix selection for 128-bit vs 64-bit.
- **No strided / gather support** —
...
RVV work because its design choices (fixed lane count + entry-time predicate,
not true VLA) are the load-bearing pattern for any scalable ISA in pystencils.

## Driver and JIT flag mapping
...
- `-march` flags:
  ...
  - `X86_SSE` → `-march=x86-64-v2`
  - `X86_AVX` → `-march=x86-64-v3`
  - `X86_AVX512[_FP16]` → `-march=x86-64-v4`(`+ -mavx512fp16`)
  - `ARM_NEON` → `-march=armv8-a`
  - `ARM_NEON_FP16` → `-march=armv8.2-a+fp16`
  - `ARM_SVE` → `-march=armv8.2-a+sve`
...
- [`submodules/pystencils/src/pystencils/include/pystencils_runtime/bits/philox_rand.h:70`](https://i10git.cs.fau.de/pycodegen/pystencils/-/blob/20d7dcf07372bf89f41f719e4bcc2bf8646a3c9f/src/pystencils/include/pystencils_runtime/bits/philox_rand.h#L70)
  ...
  — hand-written RVV-1.0 Philox kernels using `__riscv_*` intrinsics at LMUL=1
...
generates RISC-V intrinsics today.
